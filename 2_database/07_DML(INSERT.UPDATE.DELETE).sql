/*
    ...
    DML(MANIPULATION 데이터 조작어) : INSRT, UPDATE, DELETE
    DDL(DEFINITION 데이터 정의) : CREATE, ALTER, DROP
    DCL(CONTROL 데이터 제어_ : GRANT, REVOKE
...
/*
    1. INSERT
        ...
        부족하게 값을 제시할 경우, not enough values 오류 발생
...
/*
    ...
    단, 한 행 단위로 추가되므로 선택되지 않은 컬럼은 기본적으로 NULL이 들어감
    => NOT NULL 제약조건이 걸린 컬럼은 반드시 직접 값을 넣어야 한다.
...
AS (SELECT EMP_ID, EMP_NAME, DEPT_CODE, HIRE_DATE
    FROM EMPLOYEE
    WHERE 1 = 0); -- FALSE인 조건을 넣어서 데이터 없이 컬럼만 복붙
...
--전체사원의 급여를 기존 급여에 10프로 인상된 금액으로 변경(기존 급여 * 1.1)
UPDATE EMP_SALARY
...
--방명수 사원의 급여와 보너스값을 유재식 사원의 것으로 변경
UPDATE EMP_SALARY
...
/*
    ...
    [표현식]
    DELETE FROM 테이블명
    [WHERE 조건] --> WHERE절 제시안하면 전체 행이 다 삭제됨
*/
