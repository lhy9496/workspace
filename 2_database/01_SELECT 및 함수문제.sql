...
SELECT EMP_NAME, SALARY * 12 AS "연봉", (SALARY + (SALARY * NVL2(BONUS, BONUS, 0))) * 12 AS "총수령액"
        , ((SALARY + (SALARY * NVL2(BONUS, BONUS, 0))) * 12) - (SALARY * 12 * 0.03) AS "실수령액"
FROM EMPLOYEE;

--7. EMPLOYEE테이블에서 SAL_LEVEL이 S1인 사원의 이름, 월급, 고용일, 연락처 조회
SELECT EMP_NAME, SALARY, HIRE_DATE, PHONE
FROM EMPLOYEE, SAL_GRADE
WHERE (SALARY BETWEEN MIN_SAL AND MAX_SAL) AND SAL_LEVEL = 'S1';

--8. EMPLOYEE테이블에서 실수령액(6번 참고)이 5천만원 이상인 사원의 이름, 월급, 실수령액, 고용일 조회
SELECT EMP_NAME, SALARY, ((SALARY + (SALARY * NVL2(BONUS, BONUS, 0))) * 12) - (SALARY * 12 * 0.03) AS "실수령액", HIRE_DATE
FROM EMPLOYEE
WHERE (((SALARY + (SALARY * NVL2(BONUS, BONUS, 0))) * 12) - (SALARY * 12 * 0.03)) >= 50000000;
...
SELECT SUM((SALARY + (SALARY * NVL2(BONUS, BONUS, 0))) * 12)
FROM EMPLOYEE
WHERE DEPT_CODE = 'D5';
