/*
    ...
    EX)
    회원탈퇴시 기존의 회원 테이블에서 데이터를 DELETE, 곧바로 탈퇴한 회원들만 따로 보관하는 테이블에
    자동으로 INSERT 시켜야 한다.
    신고횟수가 일정 수를 넘었을 대 묵시적으로 해당 회원을 블랙리스트로 처리되게끔
    입출고에 대한 데이터가 기록(INSERT)될 때마다 해당 상품에 대한 재고수량을 매번 수정(UPDATE)해야 한다.
    
    * 트리거 종류
    -SQL문의 실행시기에 따른 분류
    >BEFORE TRIGGER : 내가 지정한 테이블에 이벤트가 발생되기 전에 트리거 실행
    >AFTER TRIGGER : 내가 지정한 테이블에 이벤트가 발생한 이후에 트리거 실행
    
    -SQL문에 의해 영향을 받는 각 행에 따른 분류
    >문장트리거 : 이벤트가 발생한 SQL문에 대해 딱 한번만 트리거 실행
    >행트리거 : 해당 SQL문을 실행할 때마다 매번 트리거 실행
                (FOR EACH ROW옵션 기술해야 함)
                >:OLE -BEFORE UPDATE(수정 전 자료), BEFORE DELETE(삭제 전 자료)
                >:NEW -AFTER UPDATE(수정 후 자료), AFTER DELETE(삭제 후 자료)
    *트리거 생성 구문
    
    [표현식]
    CREATE [OR REPLACE] TRIGGER 트리거명
    BEFORE | AFTER      INSERT | UPDATE | DELETE    ON 테이블명
    [FOR EACH ROW] <--사용한 정보 가져옴
    [DECLARE 변수선언]
    BEGIN
...
INSERT INTO TB_PRODUCT VALUES(SEQ_PCODE.NEXTVAL, '대륙폰', '샤우미', 1400000, 20);
...
--TB_PRODUCT 테이블에 매번 자동으로 재고수량이 UPDATE 되도록 트리거 작성

/*
    - 상품이 입고 -> 해당 상품을 찾아서 재고수량 증가 UPDATE
    UPDATE TB_PRODUCT
    SET STOCK = STOCK + 입고된 수량(INSERT된 자료 AMOUNT)
    WHERE PCODE = 입고 상품 번호(INSERT된 자료의 PCODE);
    
    - 상품이 출고 -> 해당 상품을 찾아서 재고수량 감소 UPDATE
    UPDATE TB_PRODUCT
    SET STOCK = STOCK - 출고된 수량(INSERT된 자료 AMOUNT)
    WHERE PCODE = 출고 상품 번호(INSERT된 자료의 PCODE);
*/
