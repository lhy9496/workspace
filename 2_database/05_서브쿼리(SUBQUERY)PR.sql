...
/*
    *서브쿼리의 구분
    서브쿼리를 수행한 결과값이 몇행 몇열로 나오냐에따라서 분류
    
    --단일행 서브쿼리 : 서브쿼리의 조회 결과값이 갯수가 오로지 1개일 때
    --다중행 서브쿼리 : 서브쿼리의 조회 결과값이 여러행일 때(여러행 한열)
    --다중열 서브워리 : 서브쿼리의 조회 결과값이 한 행이지만 컬럼이 여러개일 때
...
--3)노올철 사원의 급여보다 많이받는 사원들의 사번, 이름, 부서코드, 급여조회
...
--2_2) 대리직급이면서 위의 결과값 보다 하나라도 큰 사원 -- 3760000 2200000 2500000
...
FROM EMPLOYEE;
--> 19등이 공동이지만 그뒤에 20등이 바로 나오는 것을 알 수 있다.
SELECT *
