...
public class ArrayListRun {
	/*
	 * [컬렉션] => 자료구조 개념이 내장되어있는 클래스로 자바에서 제공하는 "자료구조"를 담당하는 "프레임워크"이다.
	 ...
	 *    배열에 새로운 값을 더 추가하고자 한다면 새로운 크기의 배열을 만들고 기존의 값을 전부 복사하는 코드를 작성해애야한다.
	 * 2) 배열 중간 위치에 값을 추가하거나 삭제하는 경우 값을 매번 땡겨주는 복잡한 코드를 직접 작성해야한다.
	 * 3) 한 공간에 한 타입의 데이터들만 저장이 가능하다.
	 * 
	 * 컬렉션의 장점
	 * 1) 크기를 지정해 줄 필요 없음. 만일 지정했다고해도 더 많은 데이터들이 들어오면 알아서 사이즈를 늘려줌. 크기 제약 X
	 * 2) 중간에 값을 추가, 삭제하는 경우 값을 땡겨주는 작업(알고리즘)을 직접 기술할 필요가 없다.
	 *    단지 메소드 호출만으로 알아서 내부적으로 진행이 된다.
	 * 3) 한 공간에 여러타입의 데이터들을 저장할 수 있다(단, 객체만 가능하다)
	...
	public static void main(String[] args) {
		ArrayList list = new ArrayList(3); // 크기를 지정할 수도 있고 안해도 괜찮다.
		...
		list.add(new Music("오늘보다 내일의 너를 더 좋아하겠지", "소각소각"));
		list.add("끝");
		
		// 지정된 크기보다 더 많이 넣어도 오류가 발생하지 않는다. => 장점1 : 크기 제약이 없다.
		// 다양한 타입의 데이터를 담을 수 있음 => 장점3 : 여러타입 보관 가능
		System.out.println(list); // list의 특징 : 순서를 유지하면서 담김(0번째 인덱스부터 차곡차곡)
		
		// 2.add(int index, E e); : 직접 인덱스를 지정해서 해당 인덱스 위치에 데이터를 추가시켜주는 메소드
		list.add(1, new Music("Ghost Twon", "Banson Boon"));
		
		// 중간 위치에 데이터 추가시 복잡한 알고리즘을 직접 구현하지 않는다 => 장점2
		...
		list.set(1, new Music("Pink Venom", "블랙핑크"));
		System.out.println(list);
		
		// 5. size() : 리스트의 사이즈를 반환시켜주는 메소드(즉, 몇개의 데이터가 담겨 있는지)
		System.out.println("리스트의 사이즈(크기) : " + list.size());
