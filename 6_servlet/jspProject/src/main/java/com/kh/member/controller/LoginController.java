...
public class LoginController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String userPwd = request.getParameter("userPsd");
		// 3) 요청하기(db에 sql문 활용해서 조회)
		// service 클래스에 담당 메서드 호출
		Member loginUser = new MemberService().loginMember(userId, userPwd);
		
		/**
		 * 응답페이지에 전달할 값이 있을 경우 어딘가에는 담아야 한다. (담을 수 있는 영역)
		...
		// 4) 처리된 결과를 가지고 사용자가 보게 될 응답뷰를 지정해서 포워딩 또는 url 재요청
		if(loginUser == null) {
			...
			//Servlet에서는 session영역에 접근하고자 한다면 우선 세션 객체를 얻어야 한다.
			HttpSession session = request.getSession();
			session.setAttribute("loginUser", loginUser);
			
			//1. 포워딩방식 => 해당방식은 url이 변경되지 않는다. => 우리는 localhost:8001/kh/라는 기존의 메인 url이 있는데
