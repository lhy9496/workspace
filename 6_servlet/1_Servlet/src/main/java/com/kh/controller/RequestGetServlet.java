...
public class RequestGetServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String name = request.getParameter("name"); // "이호연" | ""
		String gender = request.getParameter("gender"); // "M" | "W" | null
		int age = Integer.parseInt(request.getParameter("age")); // "23" => 23
		String city = request.getParameter("city"); // "서울" | "경기" 등
		double height = Double.parseDouble(request.getParameter("height")); // "170" => 170.0000
		
		//체크박스와 같이 복수의 밸류값들을 추출하고자 할 때
		String[] foods = request.getParameterValues("food"); // ["한식","중식","일식","양식","분식"] | null
		...
		//추출한 값들을 가지고 요청처리를 해야 된다(db와 상호작용)
		...
		*/
		...
		// 위의 결과에 따라 응답 페이지(html)를 만들어서 전송 
		// => 여기 Java코드 내에 사용자가 보게 될 응답 html 구문을 작성
		
		// 장점 : java 코드 내에 작성하기 때문에 반복문, 조건문, 유용한 메소드 등을 활용하여 동적인 응답을 내릴 수 있다.
		// 단점 : 불편, 복잡, 차후에 html을 수정한다면 java코드를 수정하는 것이기 때문에 수정된 내용을 다시 반영시키고자
		//		 서버를 재실행해야 된다.
		...
		// 3) 위에서 가져온 스트림을 통해서 응답 html구문을 한 줄씩 출력
		out.println("<html>");
